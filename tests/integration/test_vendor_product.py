...
async def test_vendor_product_create(api, vendor, category, profile):
    ...
    assert resp.status == 400
    result = await resp.json()
    assert result == {"errors": ["Input should be <ProductStatus.active: 'active'>: data.status"]}
    ...
    data.pop("id")
    resp = await api.post("/api/categories", json={"data": data}, auth=TEST_AUTH)
    assert resp.status == 201
...
async def test_vendor_product_with_different_formats_of_expected_values(api, vendor, mock_agreement):
    ...
    test_product["requirementResponses"] = [{"value": "IXA", "values": ["ІХА"], "requirement": "Метод аналізу"}]
    resp = await api.post(
        f'/api/vendors/{vendor["id"]}/products?access_token={vendor_token}',
        json={"data": test_product},
        auth=TEST_AUTH,
    ...
    test_product["requirementResponses"] = [
        {"values": ["ІХА"], "requirement": "Метод аналізу"},
        {"values": [95, 102, 98], "requirement": "Специфічність"},
    ]
    resp = await api.post(
        f'/api/vendors/{vendor["id"]}/products?access_token={vendor_token}',
        json={"data": test_product},
    ...
    test_product["requirementResponses"] = [
        {"values": ["ІХА"], "requirement": "Метод аналізу"},
        {"value": 96, "requirement": "Специфічність"},
        {
            "value": 25,
