...
async def test_600_image_create(api):
    ...
    data = FormData()
    data.add_field("file", file_data, filename="report.xls", content_type="application/vnd.ms-excel")
    ...
    assert {
        "url": "/static/images/aa/aa/aa/aaaaaaaaaaaaaaaaaaaaaaaaaa.png",
        "hash": "md5:71a50dbba44c78128b221b7df7bb51f1",
        "size": 95,
    } == (await resp.json())["data"]

    data = FormData()
    data.add_field("file", b"000000000", filename="report.xls", content_type="application/vnd.ms-excel")
    with patch("catalog.image.uuid4", uuid_mock):
        resp = await api.post("/api/images", data=data, auth=TEST_AUTH)
    assert resp.status == 400
    ...
    data = FormData()
    data.add_field("file", file_data * 100000, filename="report.xls", content_type="application/vnd.ms-excel")
    with patch("catalog.image.uuid4", uuid_mock):
        resp = await api.post("/api/images", data=data, auth=TEST_AUTH)
    assert resp.status == 400
    assert {"errors": ["Image must be less than 1000000 bytes"]} == await resp.json()
