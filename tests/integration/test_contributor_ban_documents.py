...
async def test_contributor_ban_doc_create(api, contributor, ban):
    ...
    resp = await api.post(
        f'/api/crowd-sourcing/contributors/{contributor["id"]}/bans/{ban["data"]["id"]}/documents',
        json={"data": doc_data},
        auth=TEST_AUTH,
    )
...
async def test_contributor_ban_doc_put(api, contributor, ban, ban_document):
    contributor = contributor["data"]
    ban = ban["data"]
    doc_before_put = ban_document["data"]
    ...
    resp = await api.put(
        f'/api/crowd-sourcing/contributors/{contributor["id"]}/bans/{ban["id"]}/documents/{doc_before_put["id"]}',
        json={"data": doc_data},
        auth=TEST_AUTH,
    )
    result = await resp.json()
...
async def test_contributor_ban_doc_patch(api, contributor, ban, ban_document):
    contributor = contributor["data"]
    ban = ban["data"]
    doc_before_patch = ban_document["data"]
