...
from tests.base import TEST_AUTH, TEST_AUTH_ANOTHER, TEST_AUTH_NO_PERMISSION
from tests.conftest import set_requirements_to_responses
from tests.utils import create_criteria, create_profile
...
async def test_110_category_create(api, mock_agreement):
    ...
    resp = await api.put("/api/categories/%s" % category_id + "-1", json=test_category, auth=TEST_AUTH)
    assert resp.status == 400, await resp.json()
    assert {"errors": ["Field required: data"]} == await resp.json()

    resp = await api.put("/api/categories/%s" % category_id + "-1", json={"data": test_category}, auth=TEST_AUTH)
    assert resp.status == 400, await resp.json()
    assert {"errors": ["id mismatch"]} == await resp.json()

    test_category["id"] = category_id[:18]

    resp = await api.put("/api/categories/%s" % category_id[:18], json={"data": test_category}, auth=TEST_AUTH)
    assert resp.status == 400, await resp.json()
    assert {"errors": ["String should match pattern '^[0-9A-Za-z_-]{20,32}$': data.id"]} == await resp.json()

    test_category["id"] = category_id

    resp = await api.put(f"/api/categories/{category_id}", json={"data": test_category}, auth=TEST_AUTH_NO_PERMISSION)
    ...
    resp = await api.put(f"/api/categories/{category_id}", json={"data": test_category}, auth=TEST_AUTH)
...
async def test_111_limit_offset(api, mock_agreement):
    ...
    for i in range(11):
        ...
        resp = await api.put(f"/api/categories/{category_id}", json={"data": test_category_copy}, auth=TEST_AUTH)
        assert resp.status == 201
        resp_json = await resp.json()
        assert "access" in resp_json
        assert "token" in resp_json["access"]
...
async def test_120_category_patch(api, category, tag):
    ...
    resp = await api.patch("/api/categories/%s" % category_id, json=patch_category_bad)
    ...
    resp = await api.patch("/api/categories/%s" % category_id, json={"data": {}}, auth=TEST_AUTH_NO_PERMISSION)
    assert resp.status == 403, await resp.json()

    patch_category_bad["access"] = category["access"]

    resp = await api.patch(
        "/api/categories/%s" % category_id, json={"data": {}, "access": category["access"]}, auth=TEST_AUTH_ANOTHER
    )
    assert resp.status == 403, await resp.json()
    assert {"errors": ["Owner mismatch"]} == await resp.json()

    resp = await api.patch("/api/categories/%s" % category_id, json=patch_category_bad, auth=TEST_AUTH)
    assert resp.status == 400, await resp.json()

    patch_category = {"data": {"title": "Респіратори та маски без клапану (приховані)", "status": "hidden"}}
    resp = await api.patch(
        "/api/categories/%s" % category_id,
        json=patch_category,
    ...
    resp = await api.patch("/api/categories/%s" % category_id, json=patch_category, auth=TEST_AUTH)
    assert resp.status == 200
    resp_json = await resp.json()
...
async def test_130_requirement_create(api, category):
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"][0] == ("Value error, expectedValues is required when dataType string: data")
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"][0] == ("Set should have at least 1 item after validation, not 0: data.expectedValues")
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"][0] == ("Value error, expectedMaxItems should be equal 1 or not exist at all: data")

    requirement_data["data"]["expectedMaxItems"] = 1
    requirement_data["data"]["unit"] = {"code": "H87", "name": "штука"}
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"][0] == ("Value error, minValue couldn't be equal or greater than maxValue: data")
...
async def test_131_requirement_patch(api, category):
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["Field required: dataType"]
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["Value error, expectedMaxItems should be equal 1 or not exist at all"]
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["Input should be greater than 0: data.expectedMinItems"]
...
async def test_140_category_agreement_id_patch(api, category):
    ...
    agreement_id = "2" * 32
    patch_category = {"access": category["access"], "data": {"agreementID": "2" * 32}}

    profile_1 = await create_profile(api, category)
    profile_2 = await create_profile(api, category, {"status": "hidden"})

    resp = await api.patch(f"/api/categories/{category_id}", json=patch_category, auth=TEST_AUTH)
    assert resp.status == 200
    resp_json = await resp.json()
...
async def test_two_criteria(api, mock_agreement):
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["Criteria classification should be unique"]
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["Criteria with this classification already exists"]
    ...
    for rg in rgs:
        ...
        rg_id = rg_data["data"]["id"]
        await api.post(
            f"/api/profiles/{profile['data']['id']}/criteria/" f"{criterion_id}/requirementGroups/{rg_id}/requirements",
