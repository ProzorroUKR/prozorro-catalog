...
async def test_contributor_ukrainian_region_dictionary(api, category):
    data = api.get_fixture_json("contributor")
    data["contributor"]["address"] = {"countryName": "Україна", "region": "невідомий"}
    resp = await api.post(
        "/api/crowd-sourcing/contributors",
        json={"data": data},
        auth=TEST_AUTH,
    ...
    assert resp.status == 400, result
    assert {
        "errors": ["Value error, must be one of classifiers/ua_regions.json: data.contributor.address.region"]
    } == result
