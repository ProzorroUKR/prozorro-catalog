...
async def test_310_profile_create(api, category):
    ...
    resp = await api.patch("/api/profiles/%s" % profile_id, json={"data": {}}, auth=TEST_AUTH)
    assert resp.status == 404
    ...
    with patch("catalog.validations.CachedSession.get") as medicine_resp:
        ...
        assert resp.status == 400
        assert {
            "errors": ["Can't get classification ATC from medicine registry, please make request later"]
        } == await resp.json()
...
async def test_311_profile_limit_offset(api, category):
    ...
    for i in range(11):
        ...
        resp = await api.put(
            f"/api/profiles/{profile_id}", json={"data": profile_copy, "access": category["access"]}, auth=TEST_AUTH
        )
        assert resp.status == 201, await resp.json()
        resp_json = await resp.json()
...
async def test_320_profile_patch(api, profile, category):
    ...
    resp = await api.patch(f"/api/profiles/{profile_id}", json=patch_profile_bad_token, auth=TEST_AUTH)
    ...
    patch_profile = {"data": {"title": "Маска (прихована)", "status": "hidden"}, "access": profile["access"]}
    ...
    with patch("catalog.state.profile.validate_agreement", return_value=AsyncMock()):
        resp = await api.patch(f"/api/profiles/{profile_id}", json=patch_profile, auth=TEST_AUTH)
        assert resp.status == 200
        resp_json = await resp.json()
        assert resp_json["data"]["agreementID"] != category["data"]["agreementID"]
...
async def test_330_requirement_create(api, category, profile_without_criteria):
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"][0] == ("Value error, expectedMinItems couldn't be greater then expectedMaxItems: data")
    ...
    requirement_data["data"] = {"title": "Виріб оснащений носовим зажимом", "dataType": "string"}
    resp = await api.post(
        f"/api/profiles/{profile_id}/criteria/{criteria_id}/requirementGroups/{rg_id}/requirements",
        json=requirement_data,
        auth=TEST_AUTH,
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == [f"requirement '{requirement_data['data']['title']}' is archived"]
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["requirement '50 штук' minValue should be equal or greater than in category"]
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["requirement '50 штук' maxValue should be equal or less than in category"]
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["requirement '50 штук' expectedValue shouldn't be more than maxValue in category"]
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["requirement '50 штук' expectedValue shouldn't be less than minValue in category"]
...
async def test_331_requirement_patch(api, profile_without_criteria):
    ...
    assert resp.status == 400
    resp_json = await resp.json()
    assert resp_json["errors"] == ["Value error, expectedMinItems couldn't be greater then expectedMaxItems"]
