...
async def test_420_product_patch(api, category, profile, product):
    ...
    patch_product_identifier = {"data": {"identifier": {"id": "0463234567819"}}, "access": product["access"]}
    ...
    patch_product["data"] = {"requirementResponses": [new_req_response, *product["data"]["requirementResponses"]]}
    resp = await api.patch(f"/api/products/{product_id}", json=patch_product, auth=TEST_AUTH)
    assert resp.status == 400
    resp_json = await resp.json()
    ...
    patch_product = {
        ...
        "access": {"owner": "cpb", "token": "321e8b0b4fc725c525d38de6e458965f"},
    }
...
async def test_product_patch_after_termination_status(api, product, category):
    ...
    patch_product = {"data": {"status": "hidden", "title": "Маски (приховані)"}, "access": product["access"]}
...
async def test_product_inactivation(api, product, category):
    ...
    patch_product = {"data": {"status": "inactive", "title": "Маски (приховані)"}, "access": product["access"]}
...
async def test_create_product_from_cabinet_category(api, mock_agreement):
    ...
    resp = await api.put(f"/api/categories/{data['id']}", json={"data": data}, auth=TEST_AUTH_CPB)
    assert resp.status == 201
...
async def test_product_document_for_localization_category(api, category):
    ...
    resp = await api.post(
        ...
        json={"data": doc_data, "access": product_access},
        auth=TEST_AUTH,
    )
    result = await resp.json()
