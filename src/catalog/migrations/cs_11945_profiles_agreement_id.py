...
from catalog.db import get_profiles_collection, init_mongo, rename_id, update_profile
...
def agreement_matches_profile(agreement, profile):
    additionalClassifications_matches = profile.get("additionalClassifications", []) == agreement.get(
    ...
    return (
        ...
        and profile["access"]["owner"] == agreement["procuringEntity"]["identifier"]["id"]
    )
...
async def migrate_profile(profile) -> Counters:
    ...
    for next_classification_id in modified_classification_ids(classification_id):
        ...
        agreements_for_profile = [a for a in agreements if agreement_matches_profile(a, profile)]
        if len(agreements_for_profile) > 1:
            logger.debug(
                f"Failed {profile['id']}. Too many agreements for {profile['id']} at {next_classification_id}."
            )
        ...
        logger.debug(
            f"Resolved {profile['id']}. Found agreement_id for profile "
...
async def load_agreements_by_classification(classification_id: str, additional_classifications_ids: List[str]):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        agreements = await asyncio.gather(
            *[load_agreement_by_id(session, agreement["id"]) for agreement in response_data["data"]]
        )
    return [a for a in agreements if a]


async def load_agreement_by_id(session, agreement_id):
    response = await session.get(f"{OPENPROCUREMENT_API_URL}/agreements/{agreement_id}")
    if response.status != 200:
        logger.info(f"Received {response.status} on {response.url}")
        return None
    response_data = await response.json()
...
async def ensure_api_accessibility():
    async with aiohttp.ClientSession() as session:
        response = await session.get(f"{OPENPROCUREMENT_API_URL}/agreements")
        if response.status != 200:
            return False
