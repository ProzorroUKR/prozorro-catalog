...
async def update_responses_in_product(product):
    ...
    category_requirements = {
        ...
        if req.get("dataType") in ("number", "integer")
    }
    responses = []
    updated = False
    for resp in product.get("requirementResponses", []):
        if resp["requirement"] in category_requirements and "values" in resp:
            if len(resp["values"]) == 1:
                resp["value"] = resp["values"][0]
                del resp["values"]
                updated = True
            elif len(resp["values"]) == 0 and "value" in resp:  # there are products with value and empty values []
                del resp["values"]
                updated = True
            else:
                INVALID_PRODUCTS.add(product["_id"])
...
async def migrate():
    ...
    logger.info(f"{len(INVALID_PRODUCTS)} PRODUCTS WITH INVALID VALUES: {INVALID_PRODUCTS}")
