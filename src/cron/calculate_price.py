...
async def calculate_price_for_product(
    ...
    def get_quartile(k: int, n: int, sorted_data: list[float]) -> Decimal:
        L = k * (n + 1) / 4.0
        if L <= 1:
            return Decimal(str(sorted_data[0]))
        if L >= n:
            return Decimal(str(sorted_data[-1]))
    ...
    for current_day in unique_days:
        ...
        amounts = sorted([float(bid['unit']['value']['amount']) for bid in window_bids])
        n = len(amounts)
